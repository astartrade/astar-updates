...
const EditArticle = ({ params }: { params: Promise<{ slug: string }> }) => {
	...
	useEffect(() => {
		const fetchArticle = async () => {
			try {
				...
				setValue(
					"publishedDate",
					data.publishedDate.toString().split("T")[0]
				);
	...
	const onSubmit = async (data: Article) => {
		// Merge featuredImage and tinyContent into the form data
		const fullData = {
			...data,
			text: tinyContent,
			featuredImage: featuredImage || article?.featuredImage, // Use the updated image or fallback to the existing one
		};
		...
		<div className='p-4'>
			<div className='container mx-auto md:p-4'>
				<Card className='max-w-4xl mx-auto rounded-none md:rounded-xl shadow-sm'>
					...
					<CardBody className='md:p-8 space-y-3'>
						<form
						...
						>
							...
							<div className='flex items-center gap-4'>
								...
								<div className='flex-1 basis-1 flex flex-col '>
									<div className=''>
										<FilePond
											className={"h-full"}
											...
											allowMultiple={false}
											name='featuredImage'
											labelIdle='Replace Featured Image:  <span class="filepond--label-action">Browse</span>'
											server={{
												process: async (
													fieldName,
													file,
													...
													try {
														const fileArrayBuffer =
															await file.arrayBuffer(); // Convert the promise to a resolved ArrayBuffer
														const fileObject =
															new File(
																[
																	fileArrayBuffer,
							...
							<Button onPress={onOpen} color="danger" size='sm' endContent={<LucideTrash2 className="text-white w-4 h-4" />} >Delete Article</Button>
